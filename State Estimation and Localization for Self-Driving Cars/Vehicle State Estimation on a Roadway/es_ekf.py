...
var_gnss  = 0.01
var_lidar = 1.00
#2adjust filter paramter to compansate noise variance
#var_imu_f = 0.1
#var_imu_w = 0.25
#var_gnss  = 0.01
#var_lidar = 100.00
#3sensor measurment are unavailable
#var_imu_f = 0.1
#var_imu_w = 0.25
#var_gnss  = 0.01
#var_lidar = 10.00
...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    deltaX_k=K_k@(y_k-p_check)

    # 3.3 Correct predicted state
    delta_P=deltaX_k[0:3]
    delta_V=deltaX_k[3:6]
    delta_Phi=deltaX_k[6:9]
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    angle=imu_w.data[k-1]*delta_t

    p_est[k]=p_est[k-1]+delta_t*v_est[k-1]+(delta_t**2)*((Cns@imu_f.data[k-1])+g)/2
    v_est[k]=v_est[k-1]+delta_t*(Cns@imu_f.data[k-1]+g)
    q_est[k]=Quaternion(axis_angle=angle).quat_mult_right(q_est[k-1])

    # 1.1 Linearize the motion model and compute Jacobians
    #skewterm=Cns@imu_f.data[k-1]
    F=np.eye(9)
    F[0:3,3:6]=delta_t*np.eye(3)
    F[3:6,6:9]=-(Cns@skew_symmetric(imu_f.data[k-1].reshape((3,1)))) * delta_t
    if k==1:
        print(F)
    Q=np.eye(6)
    Q[0,0]=Q[1,1]=Q[2,2]=var_imu_f
    Q[3,3]=Q[4,4]=Q[5,5]=var_imu_w
    Q=(delta_t**2)*Q
    # 2. Propagate uncertainty
    p_cov[k]=F@p_cov[k-1]@F.T+l_jac@Q@l_jac.T

    # 3. Check availability of GNSS and LIDAR measurements
    #if lidar.t[lidar_i] == imu_f.t[k-1]: cause IndexError: index 521 is out of bounds for axis 0 with size 521
    if lidar_i < II and lidar.t[lidar_i] == imu_f.t[k-1 ]: #k-1 imu is prediction and this one correction
        ...
        lidar_i=lidar_i+1
        print('lidar_i=',lidar_i,'lidar.t.shape[0]',lidar.t.shape[0])

    #if gnss.t[gnss_i] == imu_f.t[k-1]: cause IndexError: index 55 is out of bounds for axis 0 with size 55
    if gnss_i < JJ and gnss.t[gnss_i] == imu_f.t[k-1]:

        sensor_var=var_gnss
        y_k=(gnss.data[gnss_i]).T
        p_est[k], v_est[k], q_est[k], p_cov[k]=measurement_update(sensor_var, p_cov[k], y_k, p_est[k], v_est[k], q_est[k])
        gnss_i=gnss_i+1
        print('gnss_i=', gnss_i,'gnss.t.shape[0]',gnss.t.shape[0])
    # Update states (save)
