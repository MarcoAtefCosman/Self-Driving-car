...
# State machine states ,,define 3 states
FOLLOW_LANE = 0
...
class BehaviouralPlanner:
    ...
    def transition_state(self, waypoints, ego_state, closed_loop_speed):
        ...
        if self._state == FOLLOW_LANE:
            ...
            print('The state is follow lane')
            # ------------------------------------------------------------------
            ...
            if stop_sign_found:
                ...
                #goal is the way point of the nexi index [x y v]
                self._state = DECELERATE_TO_STOP
                print('The state is Decelrate to stop')
            # ------------------------------------------------------------------
        ...
        # the stop sign and transition to the next state.
            ...
            if self._stop_count == STOP_COUNTS:
                ...
                self._goal_index = goal_index
                self._goal_state = waypoints[self._goal_index]
                a, b = self.check_for_stop_signs(waypoints, closest_index, goal_index)
                stop_sign_found = b
    ...
    def get_goal_index(self, waypoints, ego_state, closest_len, closest_index):
        ...
        # the planner.  No need to check additional waypoints, here the arc length is the closest length
        # no need to check the next way point
        if arc_length > self._lookahead:
            return wp_index

        # We are already at the end of the path. no goals to reach
        if wp_index == len(waypoints) - 1:
            return wp_index

        # Otherwise, find our next waypoint. the coming waypoint
        # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
        # ------------------------------------------------------------------
        #if arc_length < self._lookahead:
        while wp_index < len(waypoints) - 1:
            ...
            if arc_length > self._lookahead:
                wp_index += 1
                return wp_index
                #break this was an error I return before the break
                # we had found what we search for
...
# nearest waypoint to the ego vehcile and it's index
    ...
    returns:
        [closest_len, closest_index]:
            ...
            closest_index: index of the waypoint which is closest to the vehicle.
                i.e. waypoints[closest_index] gives the waypoint closest to the vehicle.
    """
    closest_len = float('Inf') #upper value for comparison
    closest_index = 0
    # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
    # ------------------------------------------------------------------
    for i in range(len(waypoints)):
        comparison_distance = np.sqrt((ego_state[0]-waypoints[i][0])**2+(ego_state[1]-waypoints[i][1])**2)
        if comparison_distance <= closest_len:
            closest_len = comparison_distance
            closest_index = i
