    ...
    def collision_check(self, paths, obstacles):
        ...
        for i in range(len(paths)):
            ...
            for j in range(len(path[0])):
                ...
                # --------------------------------------------------------------
                # circle_locations[:, 0] = ... x
                for k in (self._circle_offsets):
                    # may be the loop isn't useful if they all have one offest,will see
                    circle_locations[:, 0] = path[0][j] + ( k * np.cos(path[2][j]) )

                # circle_locations[:, 1] = ...y
                    circle_locations[:, 1] = path[1][j] + ( k * np.sin(path[2][j]) )
    ...
    def select_best_path_index(self, paths, collision_check_array, goal_state):
        ...
        for i in range(len(paths)):
            ...
            if collision_check_array[i]:
                # here we test the first term howa far it is from the goal.
                ...
                # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                # for given path calcualte distance between it's endpoint(should be the goal) and the goal
                # --------------------------------------------------------------
                # score = ...
                x_goal = paths[i][0][len(paths[i][0])-1] #last x point at i'th path
                y_goal = paths[i][1][len(paths[i][1])-1] #last y point at i'th path

                score  = np.sqrt((goal_state[0]-x_goal)**2+(goal_state[1]-y_goal)**2)
                # --------------------------------------------------------------
                ...
                for j in range(len(paths)):
                    if j == i:
                        ...
                        if not collision_check_array[j]:
                            # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                            # Here we test how far the free path to collide path and increase the score as it's a
                            # against our criteria to choose
                            # --------------------------------------------------
                            collide_x = paths[j][0][len(paths[j][0])-1]
                            collide_y = paths[j][0][len(paths[j][0])-1]
                            bad_term  = np.sqrt((collide_x-x_goal)**2+(collide_y-y_goal)**2)
                            score += self._weight * bad_term
                            # --------------------------------------------------
